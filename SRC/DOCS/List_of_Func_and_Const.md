...
| <code>**V_DrawStrRelCentered**(*drawer* v, *int* x, *int* y, *string* str, [*int* videoflags, [*string* font]])</code> | *nil* | Same as `v.drawString()` but draws the string centered relatively to the <code>x</code> coordinate on the screen. |
